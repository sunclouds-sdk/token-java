...
public class YCToken {
    ...
    public YCToken buildToken(int appKey, String vuid) {

        if (StringUtils.isBlank(vuid)) {
            throw new TokenException("YCToken vuid can't be blank.");
        }
    ...
    public YCToken buildToken(int appKey, String vuid, int validTime) {
    ...
    public YCToken buildToken(int appKey, String vuid, int validTime, Map<String, String> parameterMap, Map<String, Long> privilegesMap) {
    ...
    public YCToken convertFromString(int appKey, String appSecret, String token)
        ...
        try {
            ...
            throw new TokenException("YCToken invalid, saveUrlBase64Decode failed", e);
        }
        ...
        try {
            ...
            if (!tokenStr.equals(token)) {
                throw new TokenException("YCToken undecoded, maybe token invalid or appkey/appSecret invalid.");
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if (this.appkey != appKey) {
                throw new TokenException(String.format("YCToken convertFromString error,appkey unequal[input:%s,intoken:%s]", appKey, String.valueOf(this.appkey)));
            }
            ...
            logger.info("YCToken[tokenVersion:" + tokenVersion + ",appkey:" + appKey + ",token:" + token + ",appSecret length:" + appSecret.length()
                    + ",vuid:" + uid + ",buildTimestampMills:" + buildTimestampMills + ",validTime:" + validTime + "]");

            return this;
        } catch (Exception e) {
            throw new TokenException("YCToken convertFromString error", e);
        }
    }
    ...
    public String convertToString(String appSecret) throws TokenException {
        ...
        try {
            ...
            throw new TokenException("product YCToken string failed", e);
        }
    ...
    public boolean validToken(int appKey, String vuid) throws TokenException {
        if (this.appkey != appKey) {
            throw new TokenException("The YCToken appKey is invalid.");
        }

        if (!this.uid.equals(vuid)) {
            throw new TokenException("The YCToken vuid is invalid.");
        }

        if (System.currentTimeMillis() > buildTimestampMills + (long) validTime * 1000) {
            throw new TokenException("The YCToken is expire because currentTime is greater than expireTime.");
        }
